...
**What I did:** Created `sql/18_joins.sql` and practiced connecting the `supplies` and `departments` tables using SQL JOINs. Used `INNER JOIN` and `LEFT JOIN` with filtering, grouping, aggregation, `HAVING`, and `ORDER BY`.
